...
export function PreviewPlayerProvider({ children }: { children: ReactNode }) {
  ...
  const [queue, setQueue] = useState<SavedSong[]>([]); const [currentIndex, setCurrentIndex] = useState(0); const [single, setSingle] = useState<SongResult | null>(null); const [isPlaying, setIsPlaying] = useState(false); const [message, setMessage] = useState<string | null>(null); const [lyricLines, setLyricLines] = useState<TimedLyricLine[]>([]); const [lyricLoading, setLyricLoading] = useState(false); const [lyricMessage, setLyricMessage] = useState<string | null>(null); const lyricRequest = useRef(0);
  ...
  function play(song: PlayableSong) { if (!song.previewUrl) return; audio.replace(song.previewUrl); audio.play(); setIsPlaying(true); setMessage(null); }
  ...
  function toggle() { if (!currentSong) return; if (isPlaying) { audio.pause(); setIsPlaying(false); } else { audio.play(); setIsPlaying(true); } }
  ...
  async function seekBy(seconds: number) { await audio.seekTo(clampPreviewSeek(audio.currentTime + seconds, status.duration || audio.duration || 30)); }
